    ...
    fun getCommonCvScoring(file: MultipartFile): CvScoringResponse {
        ...
        val message: String =
            """I have a cv and you should score it by range from 0 to 100 and respond in json with 'name',
                | 'title' and 'score' and give 'explanation'. Cv attached below $pdfPayload""".trimMargin();
        val cvScoringJson = aiClient.getCvScoring(message.plus(pdfPayload))
    ...
    fun getCvScoringBySection(file: MultipartFile): CvScoringBySectionResponse {
        ...
        val message: String =
            """I have a cv and you should score it by range from 0 to 100 by section and respond in json with 'name', 
                ...
                | which contains 'score' and 'explanation'. Cv attached below $pdfPayload""".trimMargin();
        val cvScoringJson = aiClient.getCvScoring(message.plus(pdfPayload))
